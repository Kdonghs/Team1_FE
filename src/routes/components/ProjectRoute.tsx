...
interface ProjectManager {
    ...
    export const ProjectRoute = () => {
      ...
      // 프로젝트 상세 정보 조회 먼저 시도
      const { data: projectResponse, isLoading: isProjectLoading } = useQuery<ProjectResponse, AxiosError>({
        queryKey: ["project", id],
        queryFn: async () => {
          if (!token) {
...
// 프로젝트 멤버 조회
const { data: membersResponse, isLoading: isMembersLoading } = useQuery<ProjectMembersResponse, AxiosError>({
      ...
      enabled: !!id && !!token && (!projectResponse || projectResponse.errorCode !== 200),
      staleTime: 0,
      refetchOnMount: true,
      retry: 1,
    });
